...
const Index = () => {
  ...
  const handleSelectBest = useCallback(async (messageId: string, modelIndex: number) => {
    const targetConv = conversations.find(c => c.id === activeConvId || c._id === activeConvId);
    ...
    setConversations((prev) =>
      prev.map((c) =>
        (c.id === activeConvId || c._id === activeConvId)
  ...
  const handleRetryModel = useCallback(async (messageId: string, modelName: string) => {
    ...
    setConversations(prev =>
      prev.map(c => {
        if (c.id !== activeConvId && c._id !== activeConvId) return c;
        return {
          ...c,
  ...
  }, [activeConvId, conversations, profile, benchmarkingSettings]);
