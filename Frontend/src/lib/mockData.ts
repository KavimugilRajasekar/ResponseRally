import { ChatMessage, ModelResponse, Conversation, AI_PROVIDERS, BenchmarkingMode, getComputeLevel } from "./types";
...
export function generateMockResponses(
  ...
  AI_PROVIDERS.forEach((p) => {
    if (!allowedModels || allowedModels.includes(p.name)) {
      ...
      const duration = Number((Math.random() * 2 + 1.5).toFixed(1));
